...
import { AddCalificacionExComponent } from './add-calificacion-ex/add-calificacion-ex.component';
import { ModCalificacionExComponent } from './mod-calificacion-ex/mod-calificacion-ex.component';
...
export class CalificarExamenComponent implements OnInit {
  ...
  addCalificacion(id){
    this.dialog.open(AddCalificacionExComponent,{data:id}).afterClosed().subscribe(Response=>{
      this.getactas();
    });;
  }
  modCalificacion(id,calificacion){
    ...
    this.dialog.open(ModCalificacionExComponent,{data:this.ret}).afterClosed().subscribe(Response=>{
      this.getactas();
    });;
  }

  getLectivos(){
    this.calificarS.getExsamenDocentes(localStorage.getItem('idusuario')).subscribe(Response => {
